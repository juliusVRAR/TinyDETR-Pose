...
class MosaicDetection(Dataset):
    ...
    def __init__(
        ...
        degrees=10.0, translate=0.1, mosaic_scale=(0.9, 1.1),
    ...
    def __getitem__(self, idx):
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            if self.camera_matrix is None:
                camera_matrix = None
            elif not isinstance(self.camera_matrix, dict):
                camera_matrix = self.camera_matrix
            else:
                assert isinstance(self._dataset, YCBVDataset)
                img_index = list(self._dataset.imgs_coco)[img_id]
                image_folder = self._dataset.imgs_coco[int(img_index)]['image_folder']

                if int(image_folder) < 60: # this is YCBV dataset specific
                    camera_matrix = self.camera_matrix['camera_uw']
                else:
                    camera_matrix = self.camera_matrix['camera_cmu']
