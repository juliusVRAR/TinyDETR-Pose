...
rot_rep=6d
warm_up_epochs=0
job_name="${task}_${model}_${rot_rep}_z${coef_trans_z}_r${coef_rot}_adds${coef_adds}_wu${warm_up_epochs}_obj365"
...
sbatch --job-name=$job_name \
        ...
        --export=COEF_ROT=$coef_rot,COEF_KPT=$coef_kpt,COEF_TRANS_XY=$coef_trans_xy,COEF_TRANS_Z=$coef_trans_z,COEF_ADDS=$coef_adds,COEF_CLAS=$coef_clas,COEF_BBOX=$coef_bbox,COEF_GIOU=$coef_giou,ROT_REP=$rot_rep,WARM_UP_EPOCHS=$warm_up_epochs,MODEL=$model,TASK=$task,SLURM_OUT=$slurm_out,JOB_NAME=$job_name \
        --mail-user=$mail \
