...
rootless-docker run --gpus all --shm-size=256g \
    ...
    bash -c "python /workspace/LWDETR/models/ops/setup.py build install && \
                /workspace/LWDETR/scripts/pose/$MODEL/$TASK.sh $NUM_GPUS $COEF_KPT $COEF_TRANS_XY $COEF_TRANS_Z $COEF_ROT $COEF_ADDS $COEF_CLAS $COEF_BBOX $COEF_GIOU $SLURM_JOB_ID"
